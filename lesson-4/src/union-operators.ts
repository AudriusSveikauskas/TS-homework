...
console.group('Union operators - užduotys');
{
  ...
  // TASK 3
  type Wealth = Accommodation[] | Car[];

  const wealth1: Wealth = accommodationsArr;
  const wealth2: Wealth = carsArr;
  ...
  );
  {
    ...
    const wealthArr: Wealth[] = [wealth1, wealth2];

    const printHumans = (arr: Wealth[]): void => {
      arr.forEach((wealth) => {
        wealth.forEach((obj) => {
