...
{
  type IsAdult = (obj: Person) => boolean;

  const isAdult: IsAdult = (obj) => obj.age >= 18;
...
{
  type IsFullyDescribedPerson = (obj: Person) => boolean;

  const isFullyDescribedPerson: IsFullyDescribedPerson = (obj) => !!(obj.height && obj.weight);
...
{
  type CreateInitials = (obj: Person) => string;

  const createInitials: CreateInitials = (obj) => `${obj.name[0]}${obj.surname[0]}`;
